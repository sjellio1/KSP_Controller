...
class ReferenceAscentTrajectory:
    ...
    def __init__(self):
        ...
        # Flipping these cause i want heading as a function of altitude
        self.downrange = np.linspace(0, 15363, 100)
        self.altitude = np.interp(self.downrange, points[:, 0], points[:, 1])

        self.downrange_dot = np.array(np.diff(self.downrange) / np.diff(self.altitude))
        self.downrange_dot = np.insert(self.downrange_dot, 0, [0])

    def get_dr(self, current_altitude):

        return np.interp(current_altitude, self.altitude, self.downrange)

    def get_dr_dot(self, current_altitude):

        return np.interp(current_altitude, self.altitude, self.downrange_dot)


class ReferenceLandingTrajectory:

    @staticmethod
    def update(position, velocity, t):
        x0 = position[0]
        y0 = position[1]
        z0 = position[2]
        ...
        compensated = np.array([x, x_dot, y, y_dot, z, z_dot])
